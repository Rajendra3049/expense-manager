...
| File | Role |
| ---- | ---- |
| `query-keys.ts` | Stable `queryKey` values for invalidation (`infiniteList`, `monthTotal`) |
| `use-expense-data.ts` | `useCategoriesQuery`, `useExpensesInfiniteQuery` (paged list), `useCurrentMonthExpenseTotalQuery`, insert / archive / delete mutations (archive & delete patch the infinite cache optimistically) |
